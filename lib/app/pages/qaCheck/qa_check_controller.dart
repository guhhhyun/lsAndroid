...
class QaCheckController extends GetxController with GetSingleTickerProviderStateMixin {
  ...
  RxSet<String> rightChangedRows = <String>{}.obs;
  /// 리스트
  ...
  var searchDropTextController = TextEditingController();
  RxString dayStartValue = DateFormat('yyyy-MM-dd').format(DateTime.now()).obs;

  RxMap selectedUser = {}.obs;
  ...
  RxList<dynamic> qrHeaderList = [].obs;
  ...
  RxList<dynamic> containerList = [{'CODE':'P', 'NAME': '합격'}, {'CODE':'F', 'NAME': '불합격'}].obs;
  RxMap<String, String> selectedContainer = {'CODE':'P', 'NAME': '합격'}.obs;

  RxList<dynamic> containerList2 = [{'CODE':'1', 'NAME': '조치완료'}, {'CODE':'2', 'NAME': '미완료'}].obs;
  RxMap<String, String> selectedContainer2 = {'CODE':'1', 'NAME': '조치완료'}.obs;
  RxList<dynamic> inventoryCntList = [].obs;
  RxList<dynamic> inventoryCntList2 = [].obs;

  RxList<dynamic> invCntDtList = [].obs;
  RxString invCntDt = ''.obs;

  RxList<dynamic> userList = [].obs;
  RxList<dynamic> userNameList = [].obs;
  ...
  /// 재키트 사유 입력
  TextEditingController textReKitController = TextEditingController();
  TextEditingController remarkController = TextEditingController();
  RxList<dynamic> rekitList = [].obs;
  RxBool isReKitSave = false.obs; // 재키트 저장했는지 안했는지
  ...
  RxList<dynamic> rightNoList = [].obs; // 우측 그리드 noList
  ...
  RxString itemCdDonggihwa = ''.obs;
  RxString rowSpanDonggihwa = ''.obs;
  ...
  RxInt cancelIndex = 0.obs; // 취소할 데이터의 index
  RxBool isCancelOk = false.obs;
  ...
  Future<void> registNewSave() async {
    ...
    try {
      ...
      if (retVal.resultCode == '0000') {
        if(retVal.body![0]['resultMessage'] == '') {
          userList.addAll(retVal.body![1]);
          selectedUser.addAll(userList[0]);
          Get.log('aaaaa ${selectedUser}'); 
          Map<String, int> nameCount = {};
          for (int i = 0; i < userList.length; i++) {
            String originalName = userList[i]['usrNm'] ?? '';

            if (!nameCount.containsKey(originalName)) {
              nameCount[originalName] = 1;
              // 첫 번째는 그대로 둠
            } else {
              int count = nameCount[originalName]!;
              userList[i]['usrNm'] = '$originalName$count';
...
/*          {
  ...
  Future<void> reKitSave() async {
    ...
    var params = {
      ...
      'params': [
        ...
        {
          'paramName': 'p_KIT_REMARK',
          'paramValue': textReKitController.text,
          'paramJdbcType': 'VARCHAR',
          'paramMode': 'IN'
        },
        {
  ...
  Future<void> reKitCheck() async {
    ...
    try {
      ...
      if (retVal.resultCode == '0000') {
        if(retVal.body![0]['resultMessage'] == '') {
          rekitList.addAll(retVal.body![1]);
      ...
      if (retVal.resultCode == '0000') {
        ...
        if(retVal.body![0]['returnMessage'] == '') {
          ...
          if(smallBoxDataList.isNotEmpty) {
            if(smallBoxDataList[0]['TAG_TYPE'] == '20') {
              smallBoxSave.clear();
              ...
              /*  /// 박스정보들 새로 담아서 관리
                ...
                boxWht.value = smallBoxDataList[0]['boxWht2'] == null ? '' : smallBoxDataList[0]['boxWht2'].toString();
                textMemoController.text = smallBoxDataList[0]['wrkRemark'] ?? '';
                ...
                */
              cbxMaNo.value = smallBoxDataList[0]['QR_NO'].toString();
            }
            tagType.value = smallBoxDataList[0]['TAG_TYPE'].toString();
            ...
            statusText.value = '올바른 QR코드를 다시 입력해주세요.';
          }
  ...
  Future<void> qrHeader() async {
    ...
    var params = {
      ...
      'params': [
        ...
        {
          'paramName': 'p_QA_SEQ',
          'paramValue': invCntDt.value == '' ? null : invCntDt.value,
          'paramJdbcType': 'VARCHAR',
          'paramMode': 'IN'
        },
      ]
    ...
    try {
      ...
      if (retVal.resultCode == '0000') {
        if(retVal.body![0]['resultMessage'] == '') {
          qrHeaderList.clear();
          qrHeaderList.addAll(retVal.body![1]);


          qaUser.value = qrHeaderList[0]['QA_USR'] ?? '';
          qaDt.value = qrHeaderList[0]['QA_DT'] ?? '';
          judge.value = qrHeaderList[0]['JUDGE'].toString() == 'P' ? '합격' : qrHeaderList[0]['JUDGE'].toString() == 'F' ? '불합격' : '';
          remark.value = qrHeaderList[0]['REMARK'] ?? '';
          projectNm.value = qrHeaderList[0]['PJT_NM'].toString();
          itemCdNm.value = '${qrHeaderList[0]['PITM_CD'].toString()}/${qrHeaderList[0]['ITEM_NM']}';
          itemCd.value = qrHeaderList[0]['PITM_CD'].toString();
          wrkNo.value = qrHeaderList[0]['WRK_NO'].toString();



          Get.log(qrHeaderList.toString());
          Get.log('조회 성공');
          statusText.value = '정상 조회 되었습니다.';
          isDbConnected.value = true;
        }else{
  ...
  Future<void> qaSeq() async {
    ...
    try {
      ...
      if (retVal.resultCode == '0000') {
        if(retVal.body![0]['resultMessage'] == '') {
          inventoryCntList.addAll(retVal.body![1]);
          inventoryCntList.value = inventoryCntList.reversed.toList();

          if(inventoryCntList.isNotEmpty) {
            invCntDt.value = inventoryCntList[0]['QA_SEQ'].toString();
          }

          for(var i = 0; i < inventoryCntList.length; i++) {
            invCntDtList.add(inventoryCntList[i]['QA_SEQ'].toString());
      ...
      if (retVal.resultCode == '0000') {
        if(retVal.body![0]['resultMessage'] == '') {
          ...
          for(var i = 0; i < smallBoxItemDataList.length; i++) {
            if(smallBoxItemDataList[i]['KIT_QTY'] == smallBoxItemDataList[i]['WRK_QTY']) {
              /// 색 변경 로직 시작
              isSaveColor.value = false;
              isColor.value = true;
              noList3.add('${smallBoxItemDataList[i]['ITEM_CD']}');
              //   noList3.add('${smallBoxItemDataList[i]['ROW_NO'].toString()}${smallBoxItemSaveDataList[i]['ITEM_CD']}');
      ...
      if (retVal.resultCode == '0000') {
        if(retVal.body![0]['resultMessage'] == '') {
          ...
          for(var i = 0; i < smallBoxItemSaveDataList.length; i++) {
            final name = reasonDropdownList.firstWhere((item) => item['CODE'] == smallBoxItemSaveDataList[i]['NCBX_RMK'], orElse: () => {'NAME': ''})['NAME'];
            smallBoxItemSaveDataList[i].addAll({'ncbxRmkName': name.toString()});
          }
  ...
  /// 신규
  Future<void> registMainKitQrMulti2() async {
    bLoading.value = true;
    ...
    var params = {
      ...
      'params': [
        ...
        {
          'paramName': 'p_JUDGE',
          'paramValue': selectedContainer['CODE'],
          'paramJdbcType': 'VARCHAR',
          'paramMode': 'IN'
        },
        {
  ...
  /// 구성품 스캔
  Future<void> registMainKitQr(String plag) async {
  ...
  /// 비고 저장
  Future<void> registMainKitDonggihwaNewSave() async {
    ...
    for(var i = 0; i < smallBoxItemSaveDataList.length; i++) {
      var params = {
        ...
        'params': [
          ...
          {
            'paramName': 'p_REMARK',
            'paramValue': smallBoxItemSaveDataList[i]['REMARK'],
            'paramJdbcType': 'VARCHAR',
            'paramMode': 'IN'
          },
          {
            'paramName': 'p_WRK_SEQ',
            'paramValue': smallBoxItemSaveDataList[i]['WRK_SEQ'],
            'paramJdbcType': 'VARCHAR',
            'paramMode': 'IN'
          },
          {
      ...
      if (retVal.resultCode == '0000') {
        if(retVal.body![0]['resultMessage'] == '') {
          ...
          for(var ii = 0; ii < bomDetailList.length; ii++) {
            ...
            /*   final name2 = bomConfirm.firstWhere((item) => item['CODE'] == bomDetailList[ii]['chgCfRmk'], orElse: () => {'NAME': ''})['NAME'];
            bomDetailList[ii].addAll({'chgCfRmkName': name2.toString()});*/
          }
  ...
  Future<void> deleteMainKitAddRow(int index) async {
    ...
    var params = {
      ...
      'params': [
        ...
        {
          'paramName': 'p_CBX_MA_NO',
          'paramValue': smallBoxSaveList[index]['cbxMaNo'].toString(),
          'paramJdbcType': 'VARCHAR',
          'paramMode': 'IN'
        },
        {
          'paramName': 'p_WRK_SEQ',
          'paramValue': smallBoxSaveList[index]['wrkSeq'].toString(),
          'paramJdbcType': 'VARCHAR',
          'paramMode': 'IN'
        },
        {
        ...
        smallBoxSaveList.length,
            ...
            cells: Map.from((smallBoxSaveList[index]).map(
                  (key, value) => MapEntry(key,
                      PlutoCell(
                          value: key == 'SCAN_YN' || key == 'MANUAL_YN' ? value == 'N' ? value = '' : value = 'O' : value
                      )
                  ),
            ))));
    stateManager2.removeAllRows();
...
/*  Future<void> requestPermissions() async {
  ...
  @override
  void onClose() {
    Get.log('QaCheckController - onClose !!');
    super.onClose();
  }

  @override
  void onInit() async {
    Get.log('QaCheckController - onInit !!');
    super.onInit();
    // requestPermissions();
    reqCommon();
  }
  ...
  @override
  void onReady() {
    Get.log('QaCheckController - onReady !!');
