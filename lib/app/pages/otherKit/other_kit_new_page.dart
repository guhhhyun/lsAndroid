...
class OtherKitNewPage extends StatelessWidget {
  OtherKitNewPage({super.key});
                  ...
                  for(var i = 0; i < controller.smallBoxSaveList.length; i++) {
                    if(controller.smallBoxSaveList[i]['prtNo'] == 'O') {
                      if(controller.smallBoxSaveList[i]['ncbxRmk'] != '' && controller.smallBoxSaveList[i]['ncbxRmk'] != null) {
                        controller.cbxSuNoDonggihwa.value = controller.smallBoxSaveList[i]['cbxExNo'].toString();
                        controller.cbxSuSeqDonggihwa.value = controller.smallBoxSaveList[i]['cbxExSeq'].toString();
  ...
  void syncProcess() async {
    ...
    try {
          ...
          for (var dong2 = 0; dong2 < controller.smallBoxSaveList.length; dong2++) {
            ...
            if (controller.smallBoxItemDataList[dong]['itemCd'] == controller.smallBoxSaveList[dong2]['itemCd']) {
              ...
              if (double.parse(controller.smallBoxItemDataList[dong]['cbxQty'].toString()) > double.parse(controller.smallBoxSaveList[dong2]['wrkQty'].toString())) {
                ...
                controller.smallBoxSaveList[dong2].addAll({'test': ''});
     ...
     /*   // 'sboxNo' 기준으로 마지막 항목을 식별
        var lastItems = <String, Map<String, dynamic>>{};
        for (var item in controller.smallBoxSaveList) {
          lastItems[item['sboxNo']] = item; // 동일한 'sboxNo'가 있을 경우 마지막으로 덮어씀
        }

        // 마지막 항목에 'test' 추가
        for (var item in controller.smallBoxSaveList) {
          if (lastItems[item['sboxNo']] == item) {
                      ...
                      Container(
                        ...
                        child: Center(
                          child: TextFormField(
                            ...
                            onChanged: (value) {
                              controller.popUpDataList[index].addAll({'wrkQty': double.parse(value == '' ? '0' : value)});
                            },
                          ),
                        ),
                      ),
